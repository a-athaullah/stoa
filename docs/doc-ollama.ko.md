...
|---|---|---|
| **URL** | `http://localhost:11434/v1` (로컬 데몬 경유) | `http://localhost:11434/v1` |
| **로컬 설치 필요** | 예 — Ollama CLI 필요 | 예 |
...
Ollama Cloud는 로컬 Ollama 데몬을 통해 요청을 라우팅합니다 — 머신이 Ollama 서버로 프롬프트를 전달합니다. 즉, 클라우드 모델 접근에도 **Ollama CLI 설치 및 인증이 필요**합니다.

### 1단계 — Ollama CLI 설치

위의 [로컬 Ollama 설치](#로컬-ollama-설치) 섹션을 따르세요. 데몬이 실행 중이어야 합니다.
...
프롬프트에 따라 Ollama 계정으로 인증합니다. 이렇게 하면 데몬이 접근 가능한 클라우드 모델로 요청을 라우팅할 수 있습니다.
...
**Discover 후 "N개 중 0개 usable" (모델은 목록에 있지만 모두 사용 불가)**

- **Ollama Cloud**의 경우: 데몬이 인증되지 않았습니다. 터미널에서 `ollama login`을 실행한 후 Discover Models를 다시 클릭하세요.
- Stoa는 구독 티어에서 각 모델이 사용 가능한지 확인하기 위해 테스트 요청으로 각 모델을 프로브합니다. 인증 없이는 모든 프로브가 실패하고 모델이 사용 불가로 표시됩니다.
- 로그인 후 아무것도 재시작할 필요 없습니다 — Discover Models만 다시 실행하세요.
