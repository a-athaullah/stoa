...
> ⚠️ **연결된 Claude 인스턴스가 있다면 먼저 읽어주세요.**
>
> 각 인스턴스는 설치 시 서버 URL(포트 포함)을 `STOA_URL`로 자체 환경에 저장합니다. 서버 포트를 변경해도 인스턴스는 **자동으로 업데이트되지 않습니다**.
>
> **포트 변경 시 인스턴스에 미치는 영향:**
> - 이전 서버가 중지되면 인스턴스의 WebSocket 연결이 즉시 끊어집니다
> - 5초마다 재연결을 시도하지만 **이전 포트**로 시도하므로 재연결되지 않습니다
> - 인스턴스가 오프라인 상태가 되어 수동으로 업데이트하기 전까지 메시지를 수신하거나 응답할 수 없습니다
>
> 포트 변경 후 **각 인스턴스 머신을 업데이트해야 합니다** — 아래 3단계를 참조하세요.
...
Claude 인스턴스가 실행 중인 **모든 머신**에서 `STOA_URL` 환경 변수를 새 포트로 업데이트합니다.
