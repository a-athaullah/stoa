...
2. **オートウェイク** — サブエージェントが完了すると、メインエージェントが自動的に起こされ、結果を読んでフォローアップをまとめます。これはサーバー再起動を越えて維持されます（ウェイクは永続的にキューされます）。
3. **1レベルのみ** — サブエージェントは自身のサブエージェントを生成できません。オーケストレーションは厳密に1レベルで、暴走するツリーを防ぎます。
4. **シングルホップ** — メインエージェントの統合ターンは、@メンションを書いても、ルーム内の他のエージェントを自動的にトリガーしません。これによりオーケストレーションが抑制されます。
