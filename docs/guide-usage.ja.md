...
スケジュール実行は手動トリガーと同じ経路を再利用するため、すべての安全策が有効です：ルームがアーカイブされていない・スポーン一時停止でないこと、同時実行サブエージェント上限が引き続き適用されること、親エージェントのマシンがオンラインであることが必要です。実行時にいずれかのチェックが失敗した場合、その実行はスキップされ、次回が通常どおりスケジュールされます。ダウンタイム（例：再起動）の後でも、サーバーは見逃した実行をまとめて発火することはありません — 各スケジュールは今後最大 1 回のみ実行され、見逃したスロットごとには実行されません。スケジュール実行サブエージェントも、自身のサブエージェントを生成することはできません。
