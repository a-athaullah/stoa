...
**外部プラットフォームのモデル**：**設定 > プラットフォーム**で追加のプラットフォーム（Ollama Cloud、OpenRouter など）を設定している場合、それらのモデルもセレクターにプラットフォーム名ごとにグループ化されて表示されます。
...
- **Base URL** — API エンドポイント（例：`https://api.ollama.com/v1`、`https://openrouter.ai/api/v1`）
- **API Key** — そのプロバイダーの API キー

追加すると、そのプラットフォームのモデルがルームコンポーザーの**モデルセレクター**ドロップダウンにプラットフォーム名ごとにグループ化されて表示されます。API キーはサーバー側で安全に保存され、ブラウザには送信されません。

**対応プロバイダー** — OpenAI 互換の API エンドポイントであればすべて利用可能：
- **Ollama Cloud** — `https://api.ollama.com/v1`（無料枠あり）
