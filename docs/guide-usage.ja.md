...
サブエージェントは**設定 > AIエージェント**の**サブエージェント**セクションで、エージェントごとに定義します。**+ add** をクリックして入力します：

- **ラベル**（必須）— `probe` や `researcher` のような短い名前。チャットには `Ara (probe)` のように表示されるため、どのサブエージェントがメッセージを生成したか常に分かります。
- **ティア** — `quick`、`standard`、`deep`。ティアは意図する作業の深さを示すラベルで、サブエージェントに渡され、UIに表示されます。ティア自体がモデルを選ぶわけではありません——サブエージェントは現在ルームのモデルで動作します（ティアからモデルへの自動ルーティングは計画中です）。
...
これらの制限はAPIを通じてルームごとに設定されます。専用の設定パネルは計画中です。
