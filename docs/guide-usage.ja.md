...
- **返信の監視 (Watch Replies)** *（Slack接続のみ）* — 有効にすると、このautomationをトリガーしたSlackスレッドへの返信が同じStoaスレッドに転送されます。各返信は以下の形式で届きます：

  ```
  [New Reply on Thread]: 送信者名 (送信者ID) - メッセージ内容
  ```

  エージェントはシステムプロンプトに指示がない限り、これらの返信に自動的に応答しません。同じSlackスレッドで別のautomationが発火した場合、そのメッセージは新しいスレッドを作らず既存のStoaスレッドに追加されます。デフォルト：オフ。
...
- **エージェントカード** — バックエンドタイプ、言語セレクター、名前フィールド、プラットフォームセレクター、再インストール用のインストールコマンド
