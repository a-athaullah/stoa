...
**外部プラットフォームのモデル**：**設定 > プラットフォーム**で追加のプラットフォーム（Ollama Cloud、OpenRouter など）を設定している場合、有効化されたモデルもセレクターにプラットフォーム名ごとにグループ化されて表示されます。画像入力に対応したモデルには 👁 アイコンが表示されます。
...
サイドバーの**歯車アイコン**をクリックして設定パネルを開きます。設定は6つのタブに分かれています：
...
保存後、**discover models** をクリックして、API キーでアクセス可能なモデルを検出します。Discovery ではモデルごとに vision 対応も検出され、画像入力に対応したモデルには 👁 アイコンが表示されます。
...
API キーはサーバー側で安全に保存され、ブラウザには送信されません。
