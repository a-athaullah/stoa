...
**設定 > Claude** でエージェント名をクリックするとインライン編集できます。
...
**設定 > Claude** でエージェント横の**削除ボタン**をクリックして削除します。エージェントの登録が解除され、すべてのルームから削除されます。
...
- **設定 > Claude > [エージェント名]** でエージェントの作業ディレクトリを確認
- UI または API で新しい作業ディレクトリを追加
- ルーム作成時に特定の作業ディレクトリを割り当て

### クライアントバージョニング

各エージェントは**クライアントバージョン**（例：`v0.2.2`）をサーバーに報告します。**設定 > Claude** で各エージェント名の横にバージョンが表示されます。どのエージェントが最新のクライアントコードを実行しているか確認できます。

### エージェントコントロール

**設定 > Claude** で、各エージェントには2つのアクションボタンがあります：
...
ワークスペースパネルはチャットの右側に表示されるファイルブラウザ＆エディタです。AIエージェントのマシン上のファイルを閲覧・編集できます。

### ファイル編集

テキストファイルの **Edit** ボタンをクリックして編集モードに入ります。**CodeMirror 6** によるシンタックスハイライト対応（JavaScript、Python、JSON、HTML、CSS、Markdown）。

- `Ctrl+S`（`Cmd+S`）で保存、`Ctrl+/` でコメント切替、`Ctrl+Shift+D` で行複製
- ファイルが外部で変更された場合、リロードまたは上書きのダイアログが表示
- 未保存の編集は自動的にブラウザに保存され、再度開いた時に復元プロンプトが表示

### ファイル操作（コンテキストメニュー）

ファイルツリーで右クリック（タブレットでは長押し）でコンテキストメニューを開きます：新規ファイル、新規フォルダ、名前変更、削除。
