...
任意のルートメッセージ（またはそのスレッドチップ）をクリックすると、画面右側に**スレッドパネル**が開きます。パネルにはすべての返信とスレッドへ直接返信できるコンポーザーが含まれます。AI エージェントがスレッドに返信をストリーミングし始めると、パネルが**自動的に開きます** — ただし、別のスレッドをすでに開いていない場合のみです。×ボタンまたは **Escape** キーでパネルを閉じます。
