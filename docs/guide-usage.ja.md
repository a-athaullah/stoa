...
チャットフィードは**ルートメッセージのみ**を表示します — 新しい会話スレッドを開始するメッセージです。ルートメッセージに返信がある場合、その下に**スレッドチップ**が表示され、次の情報が確認できます：

- 返信数
- 参加者のアバター
- 最終返信時刻
- そのスレッド内でエージェントがストリーミング中の場合はパルスドット

任意のルートメッセージ（またはそのスレッドチップ）をクリックすると、画面右側に**スレッドパネル**が開きます。パネルにはそのスレッドのすべての返信と、スレッドへ直接返信できるコンポーザーが含まれます。×ボタンまたは **Escape** キーでパネルを閉じます。

**ディープリンク：** ルームURLに `?thread=<メッセージID>` を付加すると、ページ読み込み時に特定のスレッドを直接開けます。
...
サブエージェントは**設定 > AIエージェント**の**サブエージェント**セクションで、エージェントごとに定義します。**+ add sub-agent** をクリックして入力します：

- **ラベル**（必須）— `probe` や `researcher` のような短い名前。チャットには `Ara (probe)` のように表示されるため、どのサブエージェントがメッセージを生成したか常に分かります。
...
- **モデルオーバーライド**（任意）— ティアのチェーンを使う代わりに、サブエージェントを特定の単一モデルに固定します。ティアのルーティングに従うには **use tier** のままにします。固定したモデルにフォールバックはなく、単独で試行されます。
...
定義済みのサブエージェントは、そのルームで使用する前にリンクする必要があります。ルームヘッダーのエージェントのシールをクリックして **「<エージェント>のサブエージェント」** を開き、そのルームで各サブエージェントをオン／オフに切り替えます。リンクされたサブエージェントのみがそのルームで生成できます。リンク後は、ラベルを@メンションすることでサブエージェントに直接呼びかけることもできます（例：`@probe`）。
...
2. **オートウェイク** — サブエージェントが完了すると、メインエージェントが自動的に起こされ、結果を読んでフォローアップをまとめます。これは直接トリガーされたものか、メンションカスケード経由かを問わず、**すべての**サブエージェント完了に適用されます。ウェイクはサーバー再起動を越えて維持されます（永続的にキューされます）。
...
サブエージェントが実行中のとき、ルームヘッダーに **「N running」** のピルが表示されます。クリックするとポップオーバーが開き、各アクティブな実行が経過時間とともに一覧表示されます。そこから次のことができます：

- 個別の実行を **Stop** — そのサブエージェントのメッセージを即座に確定します。
- **Pause spawns** — 新しいサブエージェントの開始をブロックします。すでに進行中の実行は通常どおり完了します。一時停止中はヘッダーに **「spawns paused」** のピルが表示されます。オフに切り替えると再開します。
...
### スケジュール実行サブエージェント（プロアクティブトリガー）

ルームにリンクされた永続サブエージェントは、スケジュールに従って自動的に実行できます — @メンションもオーケストレーターも不要です。定期的なチェック（例：ディスク使用量を 30 分ごとに調べる `probe`）や、固定時刻の日次レポートに便利です。

スケジュールは **Room Settings > Scheduled Triggers** で管理します。**+ add schedule** をクリックし、リンク済みサブエージェント、スケジュールタイプ（インターバルまたは毎日）、間隔または時刻、タスクを選択します。各スケジュールはその行からオン/オフの切り替え、編集、削除ができます。

スケジュールは次の 2 つの形式のいずれかです：

- **インターバル** — N 分ごとに実行（最小 5 分）。
- **日次** — 選択したタイムゾーンの固定壁時計時刻（`HH:MM`）に 1 日 1 回実行。

スケジュール実行は手動トリガーと同じ経路を再利用するため、すべての安全策が有効です：ルームがアーカイブされていない・スポーン一時停止でないこと、同時実行サブエージェント上限が引き続き適用されること、親エージェントのマシンがオンラインであることが必要です。**一時的**な理由 — 親のマシンが短時間オフライン、または同時実行上限が一時的に満杯 — で実行できない場合は、次回の予定時刻までの間、まもなく再試行されます。つまり、**日次**スケジュールはちょうどその分にマシンがオフラインだっただけで丸一日失われることはなく、マシンが復帰し次第発火し、翌日は通常の時刻に戻ります。ダウンタイム（例：再起動）の後でも、見逃したスロットが一気に再生されることは**ありません** — 各スケジュールは今後最大 1 回のみ実行され、見逃したスロットごとには実行されません。スケジュール実行サブエージェントも、自身のサブエージェントを生成することはできません。
...
ナビアイテム（上から下）：**Inbox**（ルーム一覧）、セパレーター、そして設定セクション：**AI Agent**、**Server**、**General**、**Docs**、**Platforms**、**Automation**、**Usage**、**Doctor**。
