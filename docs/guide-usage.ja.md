...
ルームヘッダーの **+** ボタンをクリックすると、エージェント追加ダイアログが開きます。AI エージェント（まだルームにいないエージェントのみ表示）を選択し、その **作業ディレクトリ** を選んで（ルーム作成時と同じ選択画面）、**Add Agent** をクリックします。これにより、ルーム内の各エージェントをそれぞれ別のディレクトリで実行できます（例：あるエージェントはプロジェクトフォルダ、別のエージェントは自身のワークスペース）。
