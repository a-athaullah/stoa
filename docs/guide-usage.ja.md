...
各ルームのヘッダーには、エージェントが使用している AI モデルを示す**モデルバッジ**が表示されます（例：「Opus 4」「Sonnet 4」）。モデルはエージェントの Claude Code 設定から自動検出されます。Gemini エージェントは現在モデルバッジが表示されません。エージェントのモデルが変更されると、バッジはリアルタイムで更新されます。
...
- **トリガーごとのチェック** — エージェントが応答するたびに、Stoa はセッションファイルのサイズを確認します。500 KB を超えている場合、エージェントは返信送信直後に `/compact` を実行し、サーバーに通知します。ユーザーは先に応答を受け取り、圧縮はバックグラウンドで行われます。
- **バックグラウンドワーカー** — 60分ごとに、エージェントはマシン上のすべての開いているセッションをスキャンし、500 KB を超えるものを圧縮します。Stoa のルームでアクティブでないローカルセッションもクリーンアップされます。
...
**設定 > オートメーション**を開き、**Connect Slack**をクリックします。2つのトークンが必要です：

- **App Token**（`xapp-1-...`）— WebSocket接続用（Socket Mode）
- **User Token**（`xoxp-...`）— チャンネルイベントの受信用

手順は[Slackセットアップガイド](doc-slack-setup)を参照してください。

### オートメーションルールの作成

Slackが接続されたら、**+ new rule**をクリックしてルールを作成します：

- **名前** — ルールの説明ラベル
- **トリガーイベント** — どのSlackイベントでルールを発火するか：`message`（公開チャンネル）、`message.groups`（プライベートチャンネル）、または`mention`
- **条件** — オプションのフィルター（AND条件）
- **対象ルーム** — プロンプトを送信するStoaルーム
- **プロンプトテンプレート** — ルームに送られるメッセージ。使用可能な変数：`{{slack_message_text}}`、`{{slack_message_link}}`、`{{slack_user}}`、`{{slack_channel}}`、`{{extracted_url}}`
...
Slackを接続し、オートメーションルールを管理。上の[オートメーション](#オートメーション)セクションを参照。
