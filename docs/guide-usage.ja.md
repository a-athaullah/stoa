...
スレッドパネルはルームコンポーザーとは独立した**専用コンポーザー**を持ちます — モデルセレクター、絵文字ピッカー、メンション補完、画像添付、スラッシュコマンドすべてが利用できます。各コンポーザーは独立した状態を持ちます：添付ファイル、引用返信、下書きはルームとスレッドで共有されません。下書きはスレッドごとに保存され、パネルを再度開くと復元されます。
...
- **セッションの圧縮** — 会話履歴を圧縮してコンテキストサイズを削減。スレッドパネルを開き、パネルヘッダーの圧縮ボタン（↕ アイコン）をクリック。圧縮中はスレッドパネル内にプログレスバーが表示され、エージェントはこれまでのコンテキストを要約してシームレスに継続します。会話が長くなりレスポンスの品質が低下し始めたときに有効
