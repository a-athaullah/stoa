...
- **作業ディレクトリを選択** — 必須。このルームで利用可能なプロジェクトコンテキストとスキルが決まります。新しいディレクトリを追加するには **+ new folder** をクリックします：**ローカルエージェント**（Stoa サーバーと同じマシンで実行中）はビジュアルな**ディレクトリブラウザ**を表示します — ブレッドクラムで移動してサブディレクトリをクリックしながら深く進み、**select** をクリックして選択を確定します。**リモートエージェント**はフリーテキスト入力を表示します。
