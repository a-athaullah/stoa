...
1. Stoa → **Settings → Automation → Connections** を開く
2. **Add Connection** をクリック
3. 名前を入力し、プロバイダーに **Slack**、トークンタイプに **User** を選択
4. **App Token**（`xapp-1-...`）と **User Token**（`xoxp-...`）を貼り付け
5. **Save** をクリック — Stoaが接続を開始します
