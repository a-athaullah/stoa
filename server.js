...
async function triggerAiResponse(roomId, ai, prompt, replyTo, attachments = [], prefetchedCtx = null) {
  ...
  const PROACTIVE_INSTRUCTIONS_SUB_AGENT = `\n\n## Progress Reporting — Mid-task Updates Only\n\nYour final text response is automatically posted to the room. Do NOT re-send it via curl — it would appear twice.\n\nUse curl only for intermediate updates during work (before your session ends):\n- After finishing your analysis and before starting main work\n- After a significant step (commit, test run, major finding)\n\nYour text response (at session end) is your final output — write it as the complete result.\n\nYou were triggered from room ${roomId}. Send all curl updates to that room — not to your own room.\n\nTo send a mid-task update:\n\`\`\`bash\nBASE_URL=$(echo "$STOA_URL" | sed "s|^ws://|http://|;s|^wss://|https://|")\ncurl -s -X POST "$BASE_URL/api/rooms/${roomId}/message" \\\n  -H "Content-Type: application/json" \\\n  -H "x-agent-id: $STOA_ACTOR_ID" \\\n  -H "x-agent-secret: $STOA_SECRET" \\\n  -d '{"content": "Your update here"}'\n\`\`\`\n\n$STOA_URL, $STOA_ACTOR_ID, and $STOA_SECRET are available as environment variables.\nRoom ID: ${roomId}`;
